import React, { useState, useCallback, useEffect, useMemo } from "react";
  ...
  const ProfileHeader = React.memo(({ userData, navigation, isOwnProfile, onFollow }) => {
    ...
    const profilePictureUrl = useMemo(() => {
      ...
      return userData.profile_picture.startsWith('http')
        ? userData.profile_picture
        : `${NGROK_URL.replace(/\/+$/, "")}/Uploads/${userData.profile_picture}`;
    }, [userData?.profile_picture]);
      ...
      try {
        ...
        while (retryCount < maxRetries) {
          const [profileResponse, postsResponse] = await Promise.all([
            fetch(profileUrl, { method: "GET", headers }).catch(e => ({ ok: false, status: 500, text: () => Promise.resolve(e.message) })),
            fetch(postsUrl, { method: "GET", headers }).catch(e => ({ ok: false, status: 500, text: () => Promise.resolve(e.message) }))
          ]);

          console.log('Profile response:', { status: profileResponse.status, ok: profileResponse.ok });
          console.log('Posts response:', { status: postsResponse.status, ok: postsResponse.ok });
          ...
          if (profileResponse.ok && postsResponse.ok) {
            ...
            if (mounted) {
              ...
              console.log('Fetched posts:', mappedPosts.length, mappedPosts);
            ...
            if (profileResponse.status === 500 || postsResponse.status === 500 || profileResponse.status === 0 || postsResponse.status === 0) {
              retryCount++;
              console.warn(`Retry ${retryCount}/${maxRetries} due to error: Profile ${profileResponse.status}, Posts ${postsResponse.status}`);
              if (retryCount === maxRetries) {
                throw new Error(`Fetch failed after retries: Profile ${profileResponse.status} - ${profileError || 'Network error'}, Posts ${postsResponse.status} - ${postsError || 'Network error'}`);
              }
              const { data: { session }, error } = await supabase.auth.refreshSession();
              ...
              console.warn('Posts endpoint returned 404, setting empty posts');
              if (mounted) {
                setUserPosts([]);
              ...
              throw new Error(`Fetch failed: Profile ${profileResponse.status} - ${profileError || 'Unknown error'}, Posts ${postsResponse.status} - ${postsError || 'Unknown error'}`);
            }
          }
        }
      } catch (error) {
        ...
        if (mounted) {
          setUserPosts([]);
          Alert.alert('Error', `Failed to fetch data: ${error.message}. Please check your network and try again.`);
          setIsLoading(false);
          setRefreshing(false);
        }
      }
              ...
              <ProfileHeader
                ...
                onFollow={async () => {
                  ...
                  try {
                    ...
                    const response = await fetch(`${baseUrl}/api/profile/follow/${userData.username}`, {
                      method,
