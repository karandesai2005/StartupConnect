...
const BusinessProfile = () => {
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      console.log('Route params:', { username, isOtherUser, isViewingOtherUser });

      // Use consistent API endpoint
      const profileUrl = `${NGROK_URL}/api/auth/profile${username ? `?username=${username}` : ''}`;
      const postsUrl = `${NGROK_URL}/api/posts/user${username ? `/${username}` : ''}`;
      ...
      if (!postsResponse.ok) {
        throw new Error(`Posts fetch failed: ${postsResponse.status} - ${await postsResponse.text()}`);
      }
      ...
      const formattedUserData = {
        ...
        isFollowing: isViewingOtherUser ? profileData.isFollowing || false : false,
      ...
      const mappedPosts = Array.isArray(postsData) ? postsData
        .map(post => ({
  ...
  useEffect(() => {
    console.log('BusinessProfile mounted, fetching data...');
    fetchInitialData();
  }, [fetchInitialData]);

  const isOwnProfile = useMemo(() => {
    if (!currentUser || !userData) {
      console.log('isOwnProfile: Waiting for data', { currentUser: currentUser?.username, userData: userData?.username });
      return false;
    }
    const { username, isOtherUser } = route.params || {};
    const result = !isOtherUser && (!username || username === currentUser.username || userData.username === currentUser.username);
    console.log('isOwnProfile calculated:', {
